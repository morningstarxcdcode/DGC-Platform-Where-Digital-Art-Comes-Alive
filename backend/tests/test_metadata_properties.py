...
from app.models import (
    ...
    Metadata,
    Provenance,
    create_metadata_from_generation,
)
...
def ipfs_url(draw):
    ...
    cid = draw(
        st.text(
            alphabet="123456789ABCDEFGHJKMNPQRSTUVWXYZabcdefghijkmnopqrstuvwxyz",
            min_size=46,
            max_size=46,
        )
    )
...
def valid_evolution(draw):
    ...
    return Evolution(parent_tokens=parent_tokens, derivation_type=derivation_type)
...
class TestMetadataProperties:
    ...
    def test_metadata_validation_fails_for_invalid_creator_address(self, metadata, invalid_creator):
        ...
        assume(
            not (
                isinstance(invalid_creator, str)
                and invalid_creator.startswith("0x")
                and len(invalid_creator) == 42
            )
        )
    ...
    @settings(suppress_health_check=[HealthCheck.function_scoped_fixture])
    def test_metadata_validation_fails_for_invalid_content_type(
        self, metadata, invalid_content_type
    ):
        """
