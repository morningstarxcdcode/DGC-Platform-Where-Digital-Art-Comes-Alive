...
class TestGenerationResultCompleteness:
    ...
    def test_each_content_type_returns_complete_result(
        self, 
        generation_service, 
        content_type
    ):
...
class TestSeedReproducibility:
    ...
    def test_same_seed_produces_same_content(self, generation_service, prompt, seed):
        ...
        assert result1.content == result2.content, \
            "Same seed should produce identical content"
    ...
    def test_different_seeds_produce_different_content(
        ...
        result1 = asyncio.get_event_loop().run_until_complete(
            generation_service.generate(request1)
        )
        
        result2 = asyncio.get_event_loop().run_until_complete(
            generation_service.generate(request2)
        )
